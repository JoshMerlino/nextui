...
import { forwardRef, useEffect, useRef, useState, type InputHTMLAttributes, type MutableRefObject, type PropsWithChildren } from "react";
...
export const classes = {
	...
	wrapper: cva("relative group/inputfield inline-flex items-center cursor-text px-4 gap-2", {
		defaultVariants: {
			variant: "outlined",
			color: "primary"
		},
		variants: {
			...
			color: {
				primary: "focus-within:border-primary focus-within:ring-primary active:border-primary active:ring-primary",
	...
	input: cva("peer bg-transparent cursor-text grow w-max", {
		defaultVariants: {
			variant: "outlined",
			size: "medium"
		},
	...
	label: cva([
		"absolute inline-flex left-4 ring-offset-white dark:ring-offset-gray-800 w-min text-gray-600 dark:text-gray-400",
		...
		variants: {
			size: {
				medium: "text-sm group-focus-within/inputfield:text-xs peer-placeholder-shown:text-xs",
				large: "text-base group-focus-within/inputfield:text-sm peer-placeholder-shown:text-sm"
			},
			color: {
...
type Props = PropsWithChildren<Omit<InputHTMLAttributes<HTMLInputElement>, "size"> & VariantProps<typeof classes[keyof typeof classes]> & Partial<{
...
export const InputField = forwardRef<HTMLInputElement, Props>(function({ className, children, label, ...props }, ref) {
	
	const defaults: Partial<typeof props> = {
		size: "medium",
	};
	...
	return (
		<label className={ cn(classes.wrapper(props as VariantProps<typeof classes.wrapper>), className) }>
			...
			<div className="flex relative h-full grow">
				...
				{ label && <p
					className={ cn(classes.label(props as VariantProps<typeof classes.label>), (hasContents || props.placeholder) && {
						"top-0": true,
						"text-sm": (props.size || defaults.size) === "large",
						"text-xs": (props.size || defaults.size) === "medium",
					}) }
