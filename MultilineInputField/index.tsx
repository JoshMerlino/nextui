...
export function MultilineInputField({ invalid = false, color = "primary", label, ...props }: TextareaHTMLAttributes<HTMLTextAreaElement> & Partial<Props>): JSX.Element {
	...
	// Input classnames
	const input = {
		"peer focus-within:outline-0 text-gray-700 dark:text-gray-200 font-roboto bg-transparent font-normal placeholder:text-gray-600 dark:placeholder:text-gray-400 text-sm w-full h-full resize-none py-4": true,
	...
	// Wrapper classnames
	const wrapper = {
		"border border-gray-400/40 dark:border-gray-400/25 ring-1 ring-transparent rounded-lg relative [&:has(:disabled)]:border-dashed flex gap-2 px-4 items-center bg-inherit transition-colors select-none cursor-text group/wrapper rounded-lg": true,
	...
	// Label classnames
	const labelStyles = {
		"absolute select-none text-gray-600 dark:text-gray-400 text-sm font-roboto font-normal pointer-events-none whitespace-nowrap transition-[top,font-size,color] bg-inherit rounded-md": true,
		...
		"top-0 -translate-y-1/2 text-xs -mx-1.5 px-1.5 !bg-red-500": hasContents,
	};
